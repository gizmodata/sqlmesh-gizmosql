...
def test_create_table_auto_creates_schema(gizmosql_adapter: GizmoSQLEngineAdapter):
    """Test that create_table automatically creates the schema if it doesn't exist.

    This addresses the error: "Schema X not found in DuckLakeCatalog Y"
    that occurs when SQLMesh tries to create a table in a non-existent schema.
    """
    ...
    try:
        ...
        # Create table WITHOUT explicitly creating the schema first
        # This should auto-create the schema
        columns_to_types = {
            "id": exp.DataType.build("INT"),
            "name": exp.DataType.build("VARCHAR"),
        }
        ...
        # Insert and query data to verify table is functional
...
def test_ctas_auto_creates_schema(gizmosql_adapter: GizmoSQLEngineAdapter):
    """Test that CTAS (create table as select) automatically creates the schema."""
    ...
    try:
        ...
        # Use CTAS without explicitly creating the schema first
        gizmosql_adapter.ctas(
            table_name,
            exp.select(
                exp.Literal.number(1).as_("id"),
                exp.Literal.string("hello").as_("name"),
            ),
        )
